...
LazyEdge giải quyết cùng nhóm vấn đề với đường hầm ngược kiểu ngrok nhưng cố ý có phạm vi hẹp hơn: v0.1 chỉ công khai các tuyến HTTP API đã duyệt, không mở cổng TCP tùy ý hay URL công khai phát sinh. Xem [các khái niệm ở quy mô lớn](../docs/concepts-at-scale.md) để hiểu bản đồ công nghệ.
...
Giao diện `v1alpha1` đang ở trạng thái preview. Phiên bản 0.1 không cung cấp `apply`, `rollback` hoặc `uninstall` từ xa: bộ render tạo artefact có thể xem xét và quản trị viên chủ động cài đặt chúng. Xem [hướng dẫn bắt đầu đầy đủ](../docs/quickstart.md).
...
**Bản xem trước v0.1.** Giao diện công khai có thể thay đổi. Kho mã này mô tả nền tảng an toàn dự kiến; nó không tuyên bố bất kỳ domain, máy chủ đám mây, đường hầm, phiên bản npm hay triển khai LocalLLM cụ thể nào đang hoạt động trước khi môi trường đó được xác minh độc lập. Không dùng LazyEdge làm lớp bảo vệ duy nhất cho hệ thống nhạy cảm hoặc quan trọng về an toàn.
