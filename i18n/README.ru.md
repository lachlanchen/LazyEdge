...
Интерфейс `v1alpha1` имеет статус preview. Версия 0.2 не предоставляет удалённые `apply`, `rollback` или `uninstall`: генераторы создают проверяемые артефакты, которые администратор устанавливает осознанно. См. [полное руководство по запуску](../docs/quickstart.md).
...
**Предварительная версия v0.2.** Публичный интерфейс может измениться. Репозиторий описывает предполагаемую безопасную основу; он не утверждает, что конкретный домен, облачный сервер, туннель, npm-версия или развёртывание LocalLLM уже работают, пока эта среда не проверена независимо. Не используйте LazyEdge как единственную защиту чувствительных или критичных для безопасности систем.
